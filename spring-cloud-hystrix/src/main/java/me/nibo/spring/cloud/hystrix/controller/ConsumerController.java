package me.nibo.spring.cloud.hystrix.controller;

import com.netflix.hystrix.strategy.concurrency.HystrixRequestContext;
import me.nibo.spring.cloud.hystrix.domain.Account;
import me.nibo.spring.cloud.hystrix.service.MyService;
...
//                Map.class, id).getBody();
        // region Hystrix 缓存测试，但感觉只能在当前线程中使用，有点怪
        HystrixRequestContext.initializeContext();
        Account result = service.getUserById(id);
        Account result2 = service.getUserById(id);
        // endregion
    ...
    public ResponseEntity<Account> saveUser(@RequestBody Account account) {
        Account result = service.addUser(account);
